...
namespace Lidgren.Network
{
	internal sealed class NetDiscoveryRequest
	{
		...
		private List<NetworkEndPoint> m_discovered;
		...
		internal NetDiscoveryRequest(NetworkEndPoint endPoint, ushort number, bool useBroadcast)
		{
			m_endPoint = endPoint;
			m_number = number;
		...
		internal NetDiscoveryRequest(NetworkEndPoint endPoint, ushort number, bool useBroadcast, float interval, float timeout)
		{
			m_endPoint = endPoint;
			m_number = number;
			m_useBroadcast = useBroadcast;
		...
		internal bool HasDiscovered(NetworkEndPoint endPoint)
		{
			if (m_discovered != null && m_discovered.Contains(endPoint))
				return true;
			if (m_discovered == null)
				m_discovered = new List<NetworkEndPoint>();
	...
	internal sealed class NetDiscovery
	{
		...
		/// </summary>
		internal void SendDiscoveryRequest(NetworkEndPoint endPoint, bool useBroadcast)
		{
			DoSendDiscoveryRequest(endPoint, useBroadcast, null, 10.0f, 30.0f);
		}

		internal void SendDiscoveryRequest(NetworkEndPoint endPoint, bool useBroadcast, float interval, float timeout)
		{
